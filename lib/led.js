...
class Led {
  ...
  constructor(pins, options) {
    ...
    const opts = Object.assign({}, options, {mode: GPIO.OUTPUT});
    this._gpios = _.map(this._pins, pin=> new GPIO(pin, opts));
  ...
  close() {
    this._reset();
    _.forEach(this._gpios, gpio => gpio.close());
  };
  ...
  _reset(interval, fn) {
    ...
    if (interval) {
      this._hinterval = setInterval(() => this._update(), interval);
    }
  };
