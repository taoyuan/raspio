...
let pi;
try {
  pi = require('wiring-pi');
  pi.setup('wpi');
} catch (e) {
  console.error('Wiring PI is not installed. Please install it:');
  console.error('');
  console.error('       npm i wiring-pi');
  console.error('');
  process.exit(1);
}

const wpi = module.exports = exports = {};

// pinModeAlt
wpi.WPI_MODE_PINS = pi.WPI_MODE_PINS;
wpi.WPI_MODE_PHYS = pi.WPI_MODE_PHYS;
wpi.WPI_MODE_GPIO = pi.WPI_MODE_GPIO;

// pinMode
wpi.INPUT = pi.INPUT;
wpi.OUTPUT = pi.OUTPUT;
wpi.PWM_OUTPUT = pi.PWM_OUTPUT;
wpi.GPIO_CLOCK = pi.GPIO_CLOCK;
wpi.SOFT_PWM_OUTPUT = pi.SOFT_PWM_OUTPUT;
wpi.SOFT_TONE_OUTPUT = pi.SOFT_TONE_OUTPUT;

// pullUpDnControl
wpi.PUD_OFF = pi.PUD_OFF;
wpi.PUD_DOWN = pi.PUD_DOWN;
wpi.PUD_UP = pi.PUD_UP;
...
// pwmSetMode
wpi.PWM_MODE_BAL = pi.PWM_MODE_BAL;
wpi.PWM_MODE_MS = pi.PWM_MODE_MS;

wpi.PI_MODEL_UNKNOWN = pi.PI_MODEL_UNKNOWN;
wpi.PI_MODEL_A = pi.PI_MODEL_A;
wpi.PI_MODEL_B = pi.PI_MODEL_B;
wpi.PI_MODEL_BP = pi.PI_MODEL_BP;
wpi.PI_MODEL_CM = pi.PI_MODEL_CM;
wpi.PI_MODEL_AP = pi.PI_MODEL_AP;
wpi.PI_MODEL_2 = pi.PI_MODEL_2;

wpi.PI_VERSION_UNKNOWN = pi.PI_VERSION_UNKNOWN;
wpi.PI_VERSION_1 = pi.PI_VERSION_1;
wpi.PI_VERSION_1_1 = pi.PI_VERSION_1_1;
wpi.PI_VERSION_1_2 = pi.PI_VERSION_1_2;
wpi.PI_VERSION_2 = pi.PI_VERSION_2;

wpi.PI_MAKER_UNKNOWN = pi.PI_MAKER_UNKNOWN;
wpi.PI_MAKER_EGOMAN = pi.PI_MAKER_EGOMAN;
wpi.PI_MAKER_SONY = pi.PI_MAKER_SONY;
wpi.PI_MAKER_QISDA = pi.PI_MAKER_QISDA;
wpi.PI_MAKER_MBEST = pi.PI_MAKER_MBEST;

wpi.PI_MODEL_NAMES = pi.PI_MODEL_NAMES;
wpi.PI_REVISION_NAMES = pi.PI_REVISION_NAMES;
wpi.PI_MAKER_NAMES = pi.PI_MAKER_NAMES;

wpi.FSEL_INPT = pi.FSEL_INPT;
wpi.FSEL_OUTP = pi.FSEL_OUTP;
wpi.FSEL_ALT0 = pi.FSEL_ALT0;
wpi.FSEL_ALT1 = pi.FSEL_ALT1;
wpi.FSEL_ALT2 = pi.FSEL_ALT2;
wpi.FSEL_ALT3 = pi.FSEL_ALT3;
wpi.FSEL_ALT4 = pi.FSEL_ALT4;
wpi.FSEL_ALT5 = pi.FSEL_ALT5;

wpi.EDGE_FALLING = pi.INT_EDGE_FALLING;
wpi.EDGE_RISING = pi.INT_EDGE_RISING;
wpi.EDGE_BOTH = pi.INT_EDGE_BOTH;
wpi.EDGE_SETUP = pi.INT_EDGE_SETUP;
...
wpi.pullUpDnControl = function pullUpDnControl(pin, pud) {
  return pi.pullUpDnControl(pin, pud);
};
...
wpi.digitalWrite = function digitalWrite(pin, state) {
  return pi.digitalWrite(pin, state);
};
...
wpi.analogWrite = function analogWrite(pin, value) {
  return pi.analogWrite(pin, value);
};
...
wpi.delayMicroseconds = function delayMicroseconds(microseconds) {
  return pi.delayMicroseconds(microseconds);
};
...
wpi.wiringPiISR = function wiringPiISR(pin, edgeType, callback) {
  return pi.wiringPiISR(pin, edgeType, callback);
};
...
wpi.wiringPiISRCancel = function wiringPiISRCancel(pin) {
  return pi.wiringPiISRCancel(pin);
};
...
wpi.setPadDrive = function setPadDrive(group, value) {
  return pi.setPadDrive(group, value);
};
...
wpi.digitalWriteByte = function digitalWriteByte(byte) {
  return pi.digitalWriteByte(byte);
};
...
wpi.pwmToneWrite = function pwmToneWrite(pin, frequency) {
  return pi.pwmToneWrite(pin, frequency);
};
...
wpi.gpioClockSet = function gpioClockSet(pin, frequency) {
  return pi.gpioClockSet(pin, frequency);
};
...
wpi.wiringPiI2CSetup = function wiringPiI2CSetup(devId) {
  return pi.wiringPiI2CSetup(devId);
};
...
wpi.wiringPiI2CSetupInterface = function wiringPiI2CSetupInterface(device, devId) {
  return pi.wiringPiI2CSetupInterface(device, devId);
};
...
wpi.wiringPiI2CReadReg8 = function wiringPiI2CReadReg8(fd, reg) {
  return pi.wiringPiI2CReadReg8(fd, reg);
};
...
wpi.wiringPiI2CReadReg16 = function wiringPiI2CReadReg16(fd, reg) {
  return pi.wiringPiI2CReadReg16(fd, reg);
};
...
wpi.wiringPiI2CWrite = function wiringPiI2CWrite(fd, data) {
  return pi.wiringPiI2CWrite(fd, data);
};
...
wpi.wiringPiI2CWriteReg8 = function wiringPiI2CWriteReg8(fd, reg, data) {
  return pi.wiringPiI2CWriteReg8(fd, reg, data);
};
...
wpi.wiringPiI2CWriteReg16 = function wiringPiI2CWriteReg16(fd, reg, data) {
  return pi.wiringPiI2CWriteReg16(fd, reg, data);
};
...
wpi.wiringPiSPIGetFd = function wiringPiSPIGetFd(channel) {
  return pi.wiringPiSPIGetFd(channel);
};
 ...
 *
 * It is possible to do simple read and writes over the SPI bus using the standard read() and write()
 * system calls though – write() may be better to use for sending data to chains of shift registers,
 * or those LED strings where you send RGB triplets of data.
 *
...
wpi.wiringPiSPIDataRW = function wiringPiSPIDataRW(channel, data) {
  return pi.wiringPiSPIDataRW(channel, data);
};
...
wpi.wiringPiSPISetup = function wiringPiSPISetup(channel, data) {
  return pi.wiringPiSPISetup(channel, data);
};
...
wpi.wiringPiSPISetupMode = function wiringPiSPISetupMode(channel, speed, mode) {
  return pi.wiringPiSPISetupMode(channel, speed, mode);
};
...
wpi.serialOpen = function serialOpen(device, baudrate) {
  return pi.serialOpen(device, baudrate);
};
...
wpi.serialPutchar = function serialPutchar(fd, character) {
  return pi.serialPutchar(fd, character);
};
...
wpi.serialPuts = function serialPuts(fd, string) {
  return pi.serialPuts(fd, string);
};
...
wpi.serialPrintf = function serialPrintf(fd, string) {
  return pi.serialPrintf(fd, string);
};
...
wpi.shiftIn = function shiftIn(dPin, cPin, order) {
  return pi.shiftIn(dPin, cPin, order);
};
...
wpi.shiftOut = function shiftOut(dPin, cPin, order, value) {
  return pi.shiftOut(dPin, cPin, order, value);
};
...
wpi.softPwmCreate = function softPwmCreate(pin, value, range) {
  return pi.softPwmCreate(pin, value, range);
};
...
wpi.softPwmWrite = function softPwmWrite(pin, value) {
  return pi.softPwmWrite(pin, value);
};
...
wpi.softServoWrite = function softServoWrite(pin, value) {
  return pi.softServoWrite(pin, value);
};
...
wpi.softServoSetup = function softServoSetup(p0, p1, p2, p3, p4, p5, p6, p7) {
  return pi.softServoSetup(p0, p1, p2, p3, p4, p5, p6, p7);
};
...
wpi.softToneWrite = function softToneWrite(pin, frequency) {
  return pi.softToneWrite(pin, frequency);
};
...
wpi.softToneStop = function softToneStop(pin) {
  return pi.softToneStop(pin);
};

/**
 * This returns the value read on the supplied analog input pin. You will need to register additional analog modules
 * to enable this function for devices such as the Gertboard, quick2Wire analog board, etc.
 *
 * @param {Number} pin
 * @returns {*}
 */
