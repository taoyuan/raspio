...
class GPIO extends EventEmitter {
  ...
  async getMode() {
    await this.$connected;
    return await PromiseA.fromCallback(cb => this._pigpio.get_mode(this._pin, cb));
  }
  ...
  async mode(mode) {
    await this.$connected;
    this._pigpio.set_mode(this._pin, mode);
  }
  ...
  async pullUpDown(pud) {
    await this.$connected;
    this._pigpio.set_pull_up_down(this._pin, pud);
  }


  async enableInterrupt(edge) {
    await this.$connected;
    return this._listener = this._pigpio.callback(this._pin, edge, (gpio, level, tick) => {
  ...
  async servoWrite(pulseWidth) {
    await this.$ready;
    this._pigpio.setServoPulsewidth(this._pin, pulseWidth);
  }

  async digitalRead() {
    await this.$ready;
    return await PromiseA.fromCallback(cb => this._pigpio.read(this._pin, cb));
  }

  async digitalWrite(level) {
    await this.$ready;
    return this._pigpio.write(this._pin, level);
  }

  async getPwmRange() {
    await this.$connected;
    return await PromiseA.fromCallback(cb => this._pigpio.get_PWM_range(this._pin, cb));
  }
  ...
  async pwmRange(range) {
    await this.$ready;
    return await this._pigpio.set_PWM_range(this._pin, range);
  }

  async getPwmDutyCycle() {
    await this.$connected;
    return await PromiseA.fromCallback(cb => this._pigpio.get_PWM_dutycycle(this._pin, cb));
  }
  ...
  async getPwmRealRange() {
    await this.$connected;
    return await PromiseA.fromCallback(cb => this._pigpio.get_PWM_real_range(this._pin, cb));
  }

  async getPwmFrequency() {
    await this.$connected;
    return await PromiseA.fromCallback(cb => this._pigpio.get_PWM_frequency(this._pin, cb));
  }
  ...
  async pwmFrequency(frequency) {
    await this.$ready;
    return await this._pigpio.set_PWM_frequency(this._pin, frequency);
  }

  async pwmWrite(dutyCycle) {
    await this.$ready;
    return this._pigpio.set_PWM_dutycycle(this._pin, dutyCycle);
  }
