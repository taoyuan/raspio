...
function Button(pin, options) {
  ...
  var btn = this._btn = new raspi.gpio(pin, raspi.gpio.OUTPUT, options.pullup ? raspi.gpio.PUD_UP : undefined);

  btn.on('interrupt', this._update.bind(this));
  btn.enableInterrupt();
}
...
Button.prototype._cancelTimer = function () {
  if (this._timer) {
